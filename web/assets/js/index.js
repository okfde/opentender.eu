...
(function () {
	...
	window.HexaFlip = (function () {
		...
		var _ref = ['Transform', 'Perspective'];
		for (var _i = 0, _len = _ref.length; _i < _len; _i++) {
			var prop = _ref[_i];
			css[prop.toLowerCase()] = prefixProp(prop);
		}

		var cssClass = baseName.toLowerCase();
		var defaults = {
			size: 25,
			margin: 10,
			perspective: 1000,
		...
		var faceNames = ['front', 'bottom', 'back', 'top', 'left', 'right'];
		var faceSequence = faceNames.slice(0, 4);
		var urlRx = /^((((https?)|(file)):)?\/\/)|(data:)|(\.\.?\/)/i;

		function HexaFlip(el, txt, options) {
			txt = txt.split('');

			var makeObject = function (a) {
				var o = {};
			...
			var getSequence = function (a, reverse, random) {
				var o = {}, p;
				for (var i = 0, l = a.length; i < l; i++) {
					if (reverse) {
						p = l - i - 1;
			...
			var me = this;
			setTimeout(function () {
				me.setValue(getSequence(txt, false));
			}, 10);
	...
	var logo = new HexaFlip(document.getElementById('header-logo'), 'OPENTENDER.EU', {
		margin: 2,
		perspective: 0,
		touchSensitivity: 5
	});
